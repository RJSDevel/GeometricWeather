...
import wangdaye.com.geometricweather.settings.ConfigStore.Companion.getInstance
...
class SearchActivityRepository constructor(context: Context,
    ...
    private val config: ConfigStore = getInstance(context, PREFERENCE_SEARCH_CONFIG)
    ...
    fun getValidWeatherSources(context: Context): List<WeatherSource> {
        val defaultSource = getInstance(context).getWeatherSource()
